...
metadata.files = MySqlBackup.json
INSTALLS += metadata

# Copy metadata during build
win32 {
    QMAKE_POST_LINK += $$QMAKE_COPY $$shell_path($$PWD/MySqlBackup.json) $$shell_path($$DESTDIR/../metadata/) $$escape_expand(\\n\\t)
} else {
    QMAKE_POST_LINK += $$QMAKE_COPY $$PWD/MySqlBackup.json $$DESTDIR/../metadata/ $$escape_expand(\\n\\t)
}