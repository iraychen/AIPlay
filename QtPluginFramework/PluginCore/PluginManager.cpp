...
bool PluginManager::initialize(const QString& pluginDir, const QString& metadataDir)
{
    ...
    LOG_INFO("PluginManager", QString("Initialized with plugin directory: %1, metadata directory: %2")
             .arg(pluginDir, metadataDir));
...
bool PluginManager::loadPlugin(const QString& pluginId)
{
    ...
    if (!metadata.isCompatibleWithFramework(m_frameworkVersion)) {
        LOG_ERROR("PluginManager", QString("Plugin %1 is not compatible with framework version %2")
                 .arg(pluginId, m_frameworkVersion));
...
bool PluginManager::unloadPlugin(const QString& pluginId)
{
    ...
    if (!dependentPlugins.isEmpty()) {
        LOG_ERROR("PluginManager", QString("Cannot unload plugin %1 because other plugins depend on it: %2")
                 .arg(pluginId, dependentPlugins.join(", ")));
        return false;
    }
...
bool PluginManager::initializePlugin(const QString& pluginId)
{
    ...
    for (const QString& depId : dependencies) {
        ...
        if (m_pluginStates[depId] != PluginState::Initialized &&
            m_pluginStates[depId] != PluginState::Active) {
...
bool PluginManager::loadPluginMetadata(const QString& pluginId)
{
    ...
    if (metadata.getPluginId() != pluginId) {
        LOG_ERROR("PluginManager", QString("Metadata ID (%1) does not match plugin ID (%2)")
                 .arg(metadata.getPluginId(), pluginId));
        return false;
    }
...
bool PluginManager::checkPluginDependencies(const QString& pluginId)
{
    ...
    for (const QString& depId : dependencies) {
        ...
        if (!depMetadata.isCompatibleWithFramework(m_frameworkVersion)) {
            LOG_ERROR("PluginManager", QString("Dependency %1 is not compatible with framework version %2")
                     .arg(depId, m_frameworkVersion));
            return false;
        }
    }
